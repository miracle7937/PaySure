...
export default function AddProvider(props) {
    ...
    const provForm = {
            abbreviation: props.provider.abbreviation,
            providerName: props.provider.providerName
    }
    ...
    const AddProvider = async(e) => {
        ...
        try {
            const user = await axios.post(url.url + '/services/providers/' + props.provider.id, provData, {
                headers: {
    ...
    return(
      ...
      <div className="app-modal-heading">
        <div className="app-modal-header">Update Provider</div>
      </div>
      <div>
          <form onSubmit={ AddProvider }>
          <input value={provData.abbreviation} onChange={ (e) => { setprovData({ ...provData, abbreviation: e.target.value  }) }} type="text" className="app-modal-form-field w-input"  placeholder="Abbreviation"  required/>
          <input value={provData.providerName} onChange={ (e) => { setprovData({ ...provData, providerName: e.target.value  }) }} type="text" className="app-modal-form-field w-input"  placeholder="Provider Name"  required/>
